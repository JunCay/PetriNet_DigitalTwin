...
class EtherSenderP(Node):
    def __init__(self, name):
        super().__init__(name)
        self.sr_command_client_ = self.create_client(SRTcpCommunication, '/ether_bridge/physical')
        self.physical_command_subscriber_ = self.create_subscription(EFMCommand, '/command/efm/physical', self.physical_command_callback, 10)
    ...
    def physical_command_callback(self, msg:EFMCommand):
        request = SRTcpCommunication.Request()
